...
uses
  SysUtils, Classes, Windows, Graphics, Messages, Forms, Menus, ShellAPI,
  FSVclBase;
...
constructor TFsTrayIcon.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TFsTrayIcon.Destroy;
begin
  SetVisible(False);
...
procedure TFsTrayIcon.DoClick;
begin
  if Assigned(fOnClick) then
    fOnClick(Self);
end;

procedure TFsTrayIcon.DoDblClick;
begin
  if Assigned(fOnDblClick) then
    fOnDblClick(Self);
end;

procedure TFsTrayIcon.DoRightClick;
var
  pt: TPoint;
begin
...
function TFsTrayIcon.MainWndMsgHook(var Msg: TMessage): Boolean;
begin
...
procedure TFsTrayIcon.Notification(AComponent: TComponent;
...
function TFsTrayIcon.GetBalloonTimeout: Integer;
begin
  Result := fData.uTimeout;
end;

procedure TFsTrayIcon.HideBalloonHint;
begin
  fData.uFlags := fData.uFlags and not NIF_INFO;
  Update;
end;

procedure TFsTrayIcon.Loaded;
begin
  inherited;
  Update;
end;

procedure TFsTrayIcon.SetIcon(const Value: TIcon);
begin
  if Value <> fIcon then
  begin
...
procedure TFsTrayIcon.SetPopupMenu(const Value: TPopupMenu);
begin
...
procedure TFsTrayIcon.SetBalloonHint(const Value: string);
begin
  fBalloonHint := Value;
  StrPLCopy(fData.szInfo, fBalloonHint, SizeOf(fData.szInfo) - 1);
end;

procedure TFsTrayIcon.SetBalloonTimeout(const Value: Integer);
begin
  fData.uTimeout := Value;
end;

procedure TFsTrayIcon.SetBalloonTitle(const Value: string);
begin
  fBalloonTitle := Value;
  StrPLCopy(FData.szInfoTitle, fBalloonTitle, SizeOf(FData.szInfoTitle) - 1);
end;

procedure TFsTrayIcon.SetDesignPreview(const Value: Boolean);
begin
...
procedure TFsTrayIcon.SetHint(const Value: string);
begin
...
procedure TFsTrayIcon.SetVisible(const Value: Boolean);
begin
...
procedure TFsTrayIcon.ShowBalloonHint;
begin
...
procedure TFsTrayIcon.BeginUpdate;
begin
  Inc(fUpdateCount);
end;

procedure TFsTrayIcon.EndUpdate;
begin
